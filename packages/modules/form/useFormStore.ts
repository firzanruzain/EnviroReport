...
export const useFormStore = create<FormStore>((set, get) => ({
  ...
  validateField: (value: any, typeId: string, config: Record<string, any>) => {
    const { fieldTypes } = get();
  ...
  formatFieldValue: (
    ...
    const { fieldTypes } = get();
    return formFieldUtils.formatFieldValue(fieldTypes, value, typeId, config);
  },
    ...
    try {
      ...
      const parsedData = {
        ...
        form_fields: data.form_fields.map((field: any) => ({
          ...field,
          config: Array.isArray(field.config) ? field.config[0] : field.config,
        })),
      };
